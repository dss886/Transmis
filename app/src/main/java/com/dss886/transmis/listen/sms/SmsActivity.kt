...
class SmsActivity : BaseSwitchActivity() {
    ...
    override fun getConfigs(): List<IConfig> {
        return mutableListOf<IConfig>().apply {
            add(SwitchConfig("短信开关", Tags.SP_SMS_ENABLE))
            ...
            add(SwitchConfig("合并长短信", Tags.SP_SMS_MERGE_LONG_TEXT))
            add(SectionConfig("提醒模版设置"))
            add(EditTextConfig("提醒标题", Tags.SP_SMS_TITLE_REGEX))
            add(EditTextConfig("提醒内容模版", Tags.SP_SMS_CONTENT_REGEX))
